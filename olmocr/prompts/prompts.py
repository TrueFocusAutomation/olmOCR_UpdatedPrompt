...
def build_openai_silver_data_prompt(base_text: str) -> str:
    return (
        ...
        f"Turn equations into a LaTeX representation, and tables into markdown format. Include all text, including headers, footers, and page numbers, but keep references and footnotes.\n"
...
def build_openai_silver_data_prompt_v2(base_text: str) -> str:
    return (
        ...
        f"Convert tables into HTML format. Include all text, including headers, footers, and page numbers, but keep references and footnotes.\n"
        f"Read any natural handwriting.\n"
...
def build_openai_silver_data_prompt_v3_simple(page_width: int, page_height: int) -> str:
    return (
        ...
        f"Include all text, including headers, footers, and page numbers, but keep references and footnotes.\n"
        f"Read any natural handwriting.\n"
        f"If there are any figures or charts, label them with the following markdown syntax ![Alt text describing the contents of the figure](page_startx_starty_width_height.png)"
        f"This is likely one page out of several in the document, so be sure to preserve any sentences that come from the previous page, or continue onto the next page, exactly as they are.\n"
        f"If there is no text at all that you think you should read, you can output null.\n"
...
def build_finetuning_prompt(base_text: str) -> str:
    return (
        ...
        f"Just return the plain text representation of this document as if you were reading it naturally, including all headers, footers, and page numbers.\n"
        f"Do not hallucinate.\n"
        f"RAW_TEXT_START\n{base_text}\nRAW_TEXT_END"
    )
...
def build_no_anchoring_yaml_prompt() -> str:
    return (
        ...
        "Just return the plain text representation of this document as if you were reading it naturally, including all headers, footers, and page numbers. Convert equations to LateX and tables to markdown.\n"
...
def build_no_anchoring_v4_yaml_prompt() -> str:
    return (
        "Attached is one page of a document that you must process. "
        "Just return the plain text representation of this document as if you were reading it naturally, including all headers, footers, and page numbers. Convert equations to LateX and tables to HTML.\n"
